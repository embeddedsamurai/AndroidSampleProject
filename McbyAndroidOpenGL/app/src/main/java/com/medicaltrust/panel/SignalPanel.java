...
public class SignalPanel extends GraphPanel{
	...
	private double[] output = new double[PNT_HEIGHT];
        private double[] drawBufferX = new double[PNT_WIDTH<PNT_HEIGHT?
                                                  PNT_HEIGHT:PNT_WIDTH];
	...
	private void drawBackground(){
		...
		for(double i = minx; i < maxx; i += userTickX){
			...
			if(j == 5){
				...
				mc.drawString(R.drawable.chars16, k + "", i, -userMemoriY*4,1);
				mc.setColor(BG_LINE_COLOR);
				j=0;
			}else{
				mc.drawLine(i, userMemoriY, i, -userMemoriY,1);	
		...
		for(double i = miny*10; i <= maxy*10 ; i += userTickY*10){
			...
			if(j == 5){
				...
				mc.drawString(R.drawable.chars16, k +"", userMemoriX, ii,1);
				mc.setColor(BG_LINE_COLOR);
				j=0;
			}else{				
				mc.drawLine(0,ii,userMemoriX,ii,1);	
	...
	private void drawWave(double dt){
		...
		//入力信号を描画
		for(int i = 0; i < drawBufferX.length - 1 ; i++){
                  // mc.drawLine(i*dt, input[i],(i + 1)*dt, input[i + 1],1);
                  drawBufferX[i] = i * dt;
		}
                mc.drawPolyline(drawBufferX, input, input.length-1, 1);
		...
		for(int i = 0; i < output.length - 1; i++){
                ...
                */
                mc.drawPolyline(drawBufferX, output, output.length-1, 1);
	}
	...
	private void drawInfoLabel(){
		...
		mc.drawString(R.drawable.chars16, infoLabel,1,0.8,1);
	}
